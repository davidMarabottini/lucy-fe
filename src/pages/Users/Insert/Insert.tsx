...
const Registration = () => {
  ...
  const userId = userIdParams ? parseInt(userIdParams, 10) : undefined;
  ...
  const {mutate: editUser, error: editError} = useUpdateUser(userId)

  const onSubmit = (values: RegistrationForm, methods: UseFormReturn<RegistrationForm>) => {
    const { dirtyFields } = methods.formState;
    const { repeatPassword: _repeatPassword, ...payload } = values;
    
    if (isEditMode) {
      const modifiedData = Object.keys(dirtyFields).reduce((acc, key) => {
        acc[key] = values[key as keyof RegistrationForm];
        return acc;
      }, {} as Partial<RegistrationForm>);

      if (values.password) {
        modifiedData.password = values.password;
      }

      editUser(modifiedData);
    } else {
      insertUser(payload);
      methods.reset();
    }
